...
public class JsonUtils {
    ...
    public static String getJsonStrByKey(String jsonStr, String key) {
        ...
        try {
            ...
            if (dataJson.has(key)) {
                String result = dataJson.getString(key);
                return StringUtils.isEmpty(result) ? result : result.replace("\\\\","");
            } else {
                return "";
            }
        }catch (Exception e) {
            e.printStackTrace();
            //Logger.e(TAG,"====================获取json串中key串异常" + e.getMessage());
            return "";
        }
    }
     ...
     * @return
     */
    public static Integer getJsonIntByKey(String jsonStr, String key) {
