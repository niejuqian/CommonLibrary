...
import com.laonie.common.dlg.CommonDialog;
...
public class UpdateAgent {
    ...
    private boolean mIsWifiOnly = false;//是否仅wifi模式下检测
    private boolean mIsCheckOnly = false;//是否仅进行检查
    ...
    private OnPromptListener mOnInstallPromptListener;
    private OnCheckFinishListener mOnCheckFinishListener;

    public UpdateAgent(Context context, String url, boolean isManual, boolean isWifiOnly,boolean isCheckOnly) {
        mContext = context;
        mUrl = url;
        mIsManual = isManual;
        ...
        mOnInstallPromptListener = new OnInstallPrompt(context);
    ...
    public void parse(String source) {
        ...
        try {
            ...
            if (!mInfo.isHasUpdate()) {
                setError(new UpdateError(UpdateError.UPDATE_NO_NEWER));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void checkFinish() {
        ...
        if (error != null) {
            ...
            }else if (mIsCheckOnly) {
                onCheckFinish(info);
            } else if (!info.isHasUpdate()) {
    ...
    public void downloadProgress(int progress) {
        if (mInfo.isSilent()) {
            Logger.e(TAG,"=======================downloadProgress=" + progress);
    ...
    public void setCheckFinishListener(OnCheckFinishListener checkFinishListener) {
        if (checkFinishListener != null) {
            mOnCheckFinishListener = checkFinishListener;
        }
    }


    private void onFailure(UpdateError error) {
        if (mIsManual || error.isError() || error.isNewVersion()) {
            mOnFailureListener.onFailure(error);
        }
    }

    private void onCheckFinish(UpdateInfo info) {
        if (null != mOnCheckFinishListener) {
            mOnCheckFinishListener.onFinish(info);
        }
    }
    ...
    private static class OnInstallPrompt implements OnPromptListener {
        private Context mContext;

        public OnInstallPrompt(Context context) {
            mContext = context;
        }
        @Override
        public void onPrompt(UpdateAgent agent) {
            ...
            DialogInterface.OnClickListener listener = new OnPromptClick(agent, true,true);
            if (info.isForce()) {
            ...
            dialog.setButton(DialogInterface.BUTTON_POSITIVE, "安装", listener);
            if (!info.isForce()){
    ...
    private static class OnPrompt implements OnPromptListener {
        ...
        public void onPrompt(UpdateAgent agent) {
            final UpdateInfo info = agent.getInfo();
            //String size = Formatter.formatShortFileSize(mContext, info.size);
            String content = String.format("最新版本：%1$s\n更新内容：\n%2$s", info.getVersionName(), info.getUpdateContent());
            CommonDialog.builder().init(mContext)
                    .setTitle(UIUtils.getString(R.string.app_upgrade_title))
                    .setContent(content)
                    .setContentGravity(Gravity.LEFT)
                    .setLeftBtnText(UIUtils.getString(R.string.btn_cancel))
                    .setLeftVisibility(info.isForce())
                    .setCancelable(false)
                    .setRightBtnText(UIUtils.getString(R.string.btn_now_upgrade))
                    .setLeftBtnListener(()->{})
                    .setRightBtnListener(() -> {
                        if (agent.getInfo().isForce()) {
                            //强制更新
                            agent.update();
                        }else {
                            //不强制更新，使用后台静默下载方式
    ...
    public static class DialogProgress implements OnProgressListener {
        ...
        @Override
        public void onFinish() {
            if (mDialog != null&&mContext!=null&&!((Activity) mContext).isFinishing()) {
                mDialog.dismiss();
                mDialog = null;
            }
        }
    ...
    public static class NotificationProgress implements OnProgressListener {
        ...
        public void onStart() {
            ...
            if (mBuilder == null) {
                String title = "下载中 - " + AppUtil.getCtx().getString(AppUtil.getCtx().getApplicationInfo().labelRes);
                mBuilder = new NotificationCompat.Builder(AppUtil.getCtx());
                mBuilder.setOngoing(true)
                        .setAutoCancel(false)
                        .setPriority(Notification.PRIORITY_MAX)
                        .setDefaults(Notification.DEFAULT_VIBRATE)
                        .setSmallIcon(AppUtil.getCtx().getApplicationInfo().icon)
                        .setTicker(title)
        ...
        @Override
        public void onProgress(int progress) {
            Logger.e(TAG,"=======================onProgress:" + progress + "---mBuilder=" + mBuilder);
            if (mBuilder != null) {
                if (progress > 0) {
                ...
                NotificationManager nm = (NotificationManager) AppUtil.getCtx().getSystemService(Context.NOTIFICATION_SERVICE);
    ...
    public static class OnPromptClick implements DialogInterface.OnClickListener {
        ...
        public OnPromptClick(UpdateAgent agent, boolean isAutoDismiss, boolean isInstall) {
            mAgent = agent;
            mIsAutoDismiss = isAutoDismiss;
