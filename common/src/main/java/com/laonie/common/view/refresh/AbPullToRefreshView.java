...
public class AbPullToRefreshView extends LinearLayout {
	...
	/** header view. */
	private AbListViewHeaderMine mHeaderView;
//	private AbListViewHeader mHeaderView;
	...
	private void addHeaderView() {
		mHeaderView = new AbListViewHeaderMine(mContext);
...
//		mFooterView.hide();
	...
	private boolean isRefreshViewScroll(int deltaY) {
		...
		if (mRecyclerView != null && mRecyclerView.getVisibility() == VISIBLE) {
			if (deltaY > 0 ){
				...
				//如果第一个显示的child是第一个，且处于最顶端，则可以进行下拉刷新
				...
				if (lastChild.getBottom() <= getHeight() && lastPosition == (count - 1)) {
		...
		// 对于ListView和GridView
		if (mAdapterView != null && mAdapterView.getVisibility() == VISIBLE) {
		...
		if (mScrollView != null && mScrollView.getVisibility() == VISIBLE) {
			...
			if (deltaY > 0 && mScrollView.getScrollY() == 0) {
				...
				if(!mEnableLoadMore) return false; // 此控件子view为ScrollView时上拉加载更多存在bug，虽设置为false但仍会出现footerView,加入此句进行限制
				mPullState = PULL_UP_STATE;
				return true;
			}
		}
	...
	public void onHeaderRefreshFinish() {
		setHeaderTopMargin(-mHeaderViewHeight);
		mHeaderView.setState(AbListViewHeader.STATE_NORMAL);
		if(mAdapterView!=null || mRecyclerView != null){
	...
	public void onFooterLoadFinish() {
		setHeaderTopMargin(-mHeaderViewHeight);
		mHeaderView.setState(AbListViewHeader.STATE_NORMAL);
		if(mAdapterView!=null || null != mRecyclerView){
			int countNew = 0;
			if (null != mAdapterView) {
				countNew = mAdapterView.getCount();
			}
			if (null != mRecyclerView) {
				countNew = mRecyclerView.getAdapter().getItemCount();
			}
			//判断有没有更多数据了
			if(countNew > mCount){
				mFooterView.setState(AbListViewFooter.STATE_READY);
	 ...
	 */
	public AbListViewHeaderMine getHeaderView() {
		return mHeaderView;
	}
