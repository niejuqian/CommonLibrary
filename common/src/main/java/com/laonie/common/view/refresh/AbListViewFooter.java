...
public class AbListViewFooter extends LinearLayout {
	...
	private void initView(Context context) {
		...
		footerView.setMinimumHeight(UIUtils.scaleValue(mContext,100));
		footerTextView = new TextView(context);
		footerTextView.setGravity(Gravity.CENTER_VERTICAL);
		setTextColor(Color.rgb(107, 107, 107));
		UIUtils.setTextSize(footerTextView,30);

		UIUtils.setPadding(footerView, 0, 10, 0, 10);
		...
		layoutParamsWW.width = UIUtils.scaleValue(mContext, 50);
		layoutParamsWW.height = UIUtils.scaleValue(mContext, 50);
		layoutParamsWW.rightMargin = UIUtils.scaleValue(mContext, 10);
	...
	public void setState(int state) {
		...
		if (state == STATE_READY) {
			...
			footerProgressBar.setVisibility(View.GONE);
//			footerTextView.setText(getResources().getString(R.string.loadingnull));
...
//			footerTextView.setText(getResources().getString(R.string.loadingnull));
			...
			footerTextView.setVisibility(View.GONE);
			footerProgressBar.setVisibility(View.GONE);
			footerTextView.setText(getResources().getString(R.string.nodata));
		}
		mState = state;
	}
